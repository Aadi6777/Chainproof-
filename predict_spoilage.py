...
# Real-world shelf life limits (from smart_manual_test.py)
REAL_SHELF_LIFE = {
...
def load_models():
    try:
        ...
        with open(os.path.join(MODEL_DIR, "shelf_life_model.pkl"), "rb") as f:
            reg = pickle.load(f)
        return encoders, scaler, clf, reg
    except Exception as e:
        print(json.dumps({"error": f"Failed to load models: {str(e)}"}))
        sys.exit(1)

def predict(input_dict, encoders, scaler, clf, reg):
    ...
    for col in CATEGORICAL:
        # Fallback to 'tomato' or 'ambient' if type is unknown to encoder
...
if __name__ == "__main__":
    try:
        ...
        result = predict(data, encoders, scaler, clf, reg)
